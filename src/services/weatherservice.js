...
const format_current_weather = (data) => {
    ...
    const {
        ...
        current: { temp_c, feelslike_c, humidity, pressure_mb, vis_km, uv, wind_kph, last_updated_epoch, condition }
    } = data;
...
const format_forecast_weather = (data) => {
    if (!data || !data.forecast || !data.location) {
        console.error("Invalid data structure for forecast weather");
